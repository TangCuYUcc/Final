...
def get_feature_opensmile(config, filepath: str) -> list:
    ...
    SmileExtract_path=os.path.join(config.opensmile_path,'bin').replace(os.path.sep,"/")
    # single_feature.csv 路径
    single_feat_path = os.path.join(BASE_DIR, config.feature_folder, 'single_feature.csv').replace(os.path.sep,"/")
    # Opensmile 配置文件路径
    opensmile_config_path = os.path.join(config.opensmile_path, 'config/is09-13', config.opensmile_config + '.conf').replace(os.path.sep,"/")

    # Opensmile 命令
    cmd = 'cd ' + SmileExtract_path + ' && SMILExtract -C ' + opensmile_config_path + ' -I ' + filepath.replace(os.path.sep,"/") + ' -O ' + single_feat_path + ' -appendarff 0'
